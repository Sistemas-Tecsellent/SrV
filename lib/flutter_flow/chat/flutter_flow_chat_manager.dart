...
class FFChatInfo {
  ...
  final List<SellersRecord> groupMembers;

  SellersRecord get currentUser => groupMembers
      .where((user) => user.reference == currentUserReference)
      .first;
  Map<String, SellersRecord> get otherUsers => Map.fromEntries(
        groupMembers
            .where((user) => user.reference != currentUserReference)
            .map((user) => MapEntry(user.reference.id, user)),
      );
  List<SellersRecord> get otherUsersList => otherUsers.values.toList();
  ...
  Stream<FFChatInfo> getChatInfo({
    ...
    return chatStream.asyncMap((chat) async {
      ...
      final groupMembers = await Future.wait(
        userRefs.map((ref) => SellersRecord.getDocument(ref).first),
      );
